...
static int read_efuses(cmd_tbl_t *cmdtp, int flag, int argc,
                       char *const argv[]) {
  struct udevice *dev;
...
static int write_efuses(cmd_tbl_t *cmdtp, int flag, int argc,
                        char *const argv[]) {
  struct udevice *dev;
  int ret;
  ...
  ret = misc_write(dev, 4, argv[1], strlen(argv[1]));
  if (ret) {
...
U_BOOT_CMD(rockchip_read_efuses, 1, 0, read_efuses,
           "Read the content of the efuses", "");

U_BOOT_CMD(rockchip_write_efuses, 2, 0, write_efuses,
           "Write the content of the efuses", "");

static int rockchip_efuse_read(struct udevice *dev, int offset, void *buf,
                               int size) {
...
static int rockchip_efuse_write(struct udevice *dev, int offset, void *buf,
                                int size) {
  struct rockchip_efuse_platdata *plat = dev_get_platdata(dev);
  struct rockchip_efuse_regs *efuse = (struct rockchip_efuse_regs *)plat->base;
  u8 *buffer = buf;
    ...
    for (int j = 0; j < NUM_BITS_IN_BYTE; ++j) {
      if (current & 1) {
        ...
        writel(readl(&efuse->ctrl) |
                   ((((bit << 5) | o) & RK3288_A_MASK) << RK3288_A_SHIFT),
               &efuse->ctrl);
        udelay(1);
